(()=>{"use strict";
const VER="6.3.0-ATOMIC-LOW-EGRESS"; const CFG=window.SUPABASE_CONFIG||{}; const BASE=(CFG.url||"").replace(/\/$/,""); const KEY=CFG.key||"";
const CODE=new URLSearchParams(location.search).get("code")||"TV-0001"; const DBN="vd_player_local_v6", MEDIA="media", META="meta";
const CHECK_MS=6*60*1000, HEART_MS=60*1000; let sessionDownloads=0; let db=null,screen=null,active=[],idx=0,currentObjectUrl=null,playing=false,scheduleTimer=null,currentPlay=null;
...
async function syncPing(){
 if(!screen)return;
 try{await rest("screen_heartbeat",{method:"POST",headers:hdr({"Prefer":"return=minimal"}),body:JSON.stringify({screen_id:screen.id,player_version:VER})})}catch(e){}
}
